...
# ── 9. effective diffusion coefficients: both limits ────────────────
print("\n9. Effective D*: coarse limit is bulk D, fine limit is the atomic mix")
from neutronics import effective_D, static_shape_solve as sss
import homogeneous_comparison as _hc
v9 = np.array([0.6744613390173657, 1 - 0.6744613390173657])
D_coarse = effective_D(phases, v9, 1e-6)
check("coarse limit: D* = bulk D",
      max(np.abs(D_coarse[p] - phases[p].D).max() for p in range(2)), 1e-6)
D_fine = effective_D(phases, v9, 1e6)
D_harm = 1.0 / (v9[0] / phases[0].D + v9[1] / phases[1].D)
check("fine limit: D* = harmonic mean (both phases)",
      max(np.abs(D_fine[p] - D_harm).max() for p in range(2)), 1e-6)
m9 = Mesh1D(H=H, N=80)
T9 = np.full((2, 80), 300.0)
k_fine = sss(phases, m9, T9, v9, mu=200.0)[0]
k_mix = _hc.static_shape_solve_hom(phases, m9, T9, v9)[0]
check("fine-mixing k equals the harmonic-D atomic mix",
      abs(k_fine - k_mix) / k_mix, 1e-4)

# ── 10. effective_K: limits (analysis only -- NOT wired into the solver) ──
print("\n10. effective_K limits (documented, not used by the conduction operator)")
from thermal import effective_K
Kb = np.array([q.K for q in phases])
K_harm10 = 1.0 / (v[0] / Kb[0] + v[1] / Kb[1])
check("default (t=None) returns bulk K",
      float(np.abs(np.array(effective_K(phases, v, 0.5)) - Kb).max()), 1e-12)
check("t -> 0: bulk K (chunks heat individually)",
      float(np.abs(np.array(effective_K(phases, v, 0.5, 1e-6)) - Kb).max()) / Kb.max(), 1e-6)
check("t -> infinity: harmonic mean (mixture homogenises)",
      float(np.abs(np.array(effective_K(phases, v, 0.5, 1e12)) - K_harm10).max()) / K_harm10, 1e-4)
