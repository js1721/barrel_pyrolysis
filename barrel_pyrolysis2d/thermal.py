...
def solve_thermal_step(phases: list,
    ...
    for p, ph in enumerate(phases):
        ...
        # relaxational: scalar microstructural exchange (mu_ex^2, equal to
        # 1D for the same medium); document closure keeps its own |m|^2.
        m2_ex = (np.maximum(mr, mz))**2 if closure == "relaxational" else m2
        coef = v_oth * (ph.K * v_oth + ph_oth.K * v_self) * m2_ex
