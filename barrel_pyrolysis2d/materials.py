...
class DelayedGroup:
    ...
    Kept for the 2D tree only: kinetics.py consumes the delayed data as
    a list of objects rather than as the BETA_I/LAMBDA_I arrays the 1D
    tree uses. DELAYED_GROUPS below is built FROM those arrays, so the
...
DELAYED_GROUPS = [
    DelayedGroup(float(b), float(l))
    for b, l in zip(BETA_I, LAMBDA_I)
]
...
# The 2D tree's existing modules (solver, thermal, neutronics,
# stochastics) refer to the phase dataclass as `Material`, the 1D
# tree as `Phase`. They are now the same object; the alias keeps the
# 2D call sites working without a tree-wide rename.
Material = Phase
