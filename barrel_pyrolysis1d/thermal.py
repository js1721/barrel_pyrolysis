...
def effective_K(phases: list, v: np.ndarray, mu, t: float = None) -> list:
    """
    Effective conductivities K_i* for the conditional-average conduction
    operator -- the thermal counterpart of neutronics.effective_D.

    NOT WIRED IN. The conduction operator still uses the bulk K_i; this
    function records the analysis and the limits, and is exercised by
    operator_tests. Read the caveat below before enabling it.

    THE DEFECT IS REAL, and the same as the neutronic one: at T_1 = T_2
    the volume-weighted phase equations leave sum_i v_i K_i, an
    ARITHMETIC mix, whereas a layered medium conducts across its layers
    with the HARMONIC mean K_harm = 1/(v_1/K_1 + v_2/K_2) -- it is the
    heat FLUX that is continuous at an interface, not the gradient. With
    K_1/K_2 = 4 here that overstates conduction.

    WHY IT CANNOT BE FIXED FROM THE MICROSTRUCTURE ALONE. The neutronic
    case interpolates on chunk size vs the diffusion length
    sqrt(D/Sigma_rem), a length the material supplies independently of
    the mixing scale. Conduction has no such length. Using the model's
    own intrinsic timescale -- the inter-phase equilibration time
    1/lambda_c, with lambda_c proportional to mu^2 -- gives a penetration
    depth sqrt(alpha/lambda_c) that scales as 1/mu, exactly like the mean
    chord: the ratio is SCALE-INVARIANT, so it yields a fixed w (~0.35
    for these materials) and never reaches either limit. The comparison
    length must therefore be macroscopic: in a transient, the penetration
    depth delta_i = sqrt(alpha_i t).

    With t given this returns that form,

        w = 1/(1 + (lambda_i/delta_i)^2),   K_i* = (1-w) K_i + w K_harm,

    which is bulk K at t -> 0 (each chunk heats on its own) and harmonic
    as t grows (the mixture homogenises). That interpolation is
    UNVALIDATED: the existing thermal benchmark is an isolated slab with
    no macroscopic gradient, so it tests the exchange, not conduction. A
    heated-slab benchmark comparing conditional T_i(z,t) against
    realisations is what would settle it -- and it also brings history
    dependence into a per-step coefficient, which needs thought.

    With t = None (the default) it returns the bulk K_i unchanged;
    mu is the inverse correlation length (cm^-1), used for the chords.
    """
    K = [float(ph.K) for ph in phases]
    if t is None or t <= 0.0:
        return K
    K_harm = 1.0 / (v[0] / K[0] + v[1] / K[1])
    out = []
    for p, ph in enumerate(phases):
        alpha = K[p] / (ph.rho * ph.Cp)
        delta = np.sqrt(alpha * t)
        mu_m = float(np.mean(np.atleast_1d(mu))) / CM_TO_M      # cm^-1 -> m^-1
        lam = 1.0 / (max(mu_m, 1e-30) * max(v[1 - p], 1e-30))   # mean chord, m
        w = 1.0 / (1.0 + (lam / delta)**2)
        out.append((1.0 - w) * K[p] + w * K_harm)
    return out
...
def stochastic_self_operator(ph: Phase,
    ...
    K = ph.K if K_eff is None else float(K_eff)
...
def solve_thermal_step(phases: list,
    ...
    for p, ph in enumerate(phases):
        ...
        # K_eff (optional): effective conductivities for CONDUCTION only;
        # the exchange term below keeps the bulk K (see effective_K).
        L  = stochastic_self_operator(ph, mesh, mu, v_k, closure,
                                      None if K_eff is None else K_eff[p])
        AB = -0.5 * dt / rCp * L
        AB[1, :] += 1.0
